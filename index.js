...
async function analyzeForCommands(userMessage, senderName) {
    try {
        const commandPrompt = `Analyze if this message is a bot command request. Respond ONLY with valid JSON.

Available commands:
- kick: Remove user from group (needs @mention or "him/her" if replying)
- poll: Create poll (extract question and options)
- avatar: Get profile picture (needs username/mention)
- purge: Delete messages (needs number)
- welcome: Welcome user (needs username/mention)

Examples:
"kick this person" → {"isCommand": true, "commandType": "kick", "target": "mentioned_user"}
"create poll about food with pizza burger sushi" → {"isCommand": true, "commandType": "poll", "question": "food", "options": ["pizza", "burger", "sushi"], "multiSelect": false}
"make multi poll for favorites with red blue green" → {"isCommand": true, "commandType": "poll", "question": "favorites", "options": ["red", "blue", "green"], "multiSelect": true}
"get john's avatar" → {"isCommand": true, "commandType": "avatar", "target": "john"}
"delete 5 messages" → {"isCommand": true, "commandType": "purge", "count": 5}
"welcome the new guy" → {"isCommand": true, "commandType": "welcome", "target": "mentioned_user"}

If NOT a command, return: {"isCommand": false}
...
User message: "${userMessage}"`;
        ...
        const commandAnalysis = JSON.parse(jsonResponse);
...
async function executeNaturalCommand(commandData, message, chat) {
    try {
        const { commandType, target, question, options, multiSelect, count } = commandData;
        
        switch (commandType) {
            case 'kick':
                // Handle kick command
                if (!chat.isGroup) {
                    return await message.reply('Kick command only works in groups.');
                }
                
                let userToKick = null;
                if (message.mentionedIds && message.mentionedIds.length > 0) {
                    userToKick = message.mentionedIds[0];
                } else if (message.hasQuotedMsg && (target === 'mentioned_user' || target === 'him' || target === 'her')) {
                    const quotedMsg = await message.getQuotedMessage();
                    userToKick = quotedMsg.author || quotedMsg.from;
                }
                
                if (!userToKick) {
                    return await message.reply('I need to know who to kick. Please mention them or reply to their message.');
                }
                
                // Execute kick logic (similar to existing kick command)
                const kickAuthorId = message.author;
                const kickSenderIsAdmin = chat.participants.find(p => p.id._serialized === kickAuthorId)?.isAdmin;
                const kickSenderIsOwner = kickAuthorId === '917428233446@c.us';

                if (!kickSenderIsAdmin && !kickSenderIsOwner) {
                    return await message.reply('You need to be a group admin to kick users.');
                }
                ...
                if (!botIsAdmin) {
                    return await message.reply('I need to be an admin to kick users.');
                }

                if (userToKick === '917428233446@c.us') {
                    return await message.reply("I'm not allowed to do that.");
                }

                const participantToKick = chat.participants.find(p => p.id._serialized === userToKick);
                if (!participantToKick) {
                    return await message.reply("That user isn't in this group.");
                }

                if (participantToKick.isAdmin || participantToKick.isSuperAdmin) {
                    return await message.reply("I can't remove another admin.");
                }

                await chat.removeParticipants([userToKick]);
                return await message.reply('✅ User removed from group.');
                
            case 'poll':
                // Handle poll creation
                if (!question || !options || options.length < 2) {
                    return await message.reply('I need a question and at least 2 options for the poll.');
                }
                
                if (options.length > 12) {
                    return await message.reply('Maximum 12 options allowed per poll.');
                }
                
                const pollOptions = {
                    allowMultipleAnswers: multiSelect || false
                };
                
                const poll = new Poll(question, options, pollOptions);
                await chat.sendMessage(poll);
                
                const pollType = multiSelect ? 'multi-select' : 'single-select';
                console.log(`📊 Created ${pollType} poll via natural language: "${question}"`);
            ...
            case 'avatar':
                ...
                if (message.mentionedIds && message.mentionedIds.length > 0) {
                    ...
                    return await message.reply('I need to know whose avatar to get. Please mention them.');
                }
                ...
                if (!profilePicUrl) {
                    return await message.reply(`${contactName} doesn't have a profile picture or it's not visible.`);
                }
            ...
            case 'purge':
                ...
                if (!chat.isGroup) {
                    return await message.reply('Purge command only works in groups.');
                }
                
                const purgeAuthorId = message.author;
                const purgeSenderIsAdmin = chat.participants.find(p => p.id._serialized === purgeAuthorId)?.isAdmin;
                const purgeSenderIsOwner = purgeAuthorId === '917428233446@c.us';

                if (!purgeSenderIsAdmin && !purgeSenderIsOwner) {
                    return await message.reply('You need to be a group admin to purge messages.');
                }

                const deleteCount = count || 5; // Default to 5 if not specified
                ...
                return await message.reply(`✅ Understood: purge ${deleteCount} messages. (Use ?purge ${deleteCount} for full implementation)`);
            ...
            case 'welcome':
                ...
                } else if (target && target !== 'mentioned_user') {
                    userName = target;
                } else {
                    return await message.reply('I need to know who to welcome. Please mention them or say their name.');
                }
                ...
                return await message.reply(welcomeMessage);
                
            default:
                return await message.reply("I recognized that as a command but couldn't execute it. Try using the traditional command format.");
        }
        
    } catch (error) {
        console.error('❌ Error executing natural command:', error);
        return await message.reply('Sorry, something went wrong executing that command.');
    }
}
    ...
    try {
        ...
        // HELP COMMAND
            ...
            const helpMessage = `Chotu helper commands\n\n` +
                ...
                `🤖 NATURAL LANGUAGE:\n` +
                `   You can also use natural language for commands!\n` +
                `   Example: "chotu kick this person", "create a poll about food"\n` +
                `   "get john's avatar", "delete 5 messages", "welcome new guy"\n\n`;
...
📱 Ready: ${isReady ? 'Yes' : 'No'}`;
        ...
        // CHECK FOR REPLIES TO BOT OR MENTIONS
            ...
            if (replyCheck.isReply || mentionCheck) {
                ...
                // First, check if this is a natural language command
                console.log(`🧠 Analyzing for commands: ${message.body}`);
                const commandAnalysis = await analyzeForCommands(message.body, senderName);
                
                if (commandAnalysis.isCommand) {
                    console.log(`⚡ Detected natural language command: ${commandAnalysis.commandType}`);
                    // Execute the command
                    await executeNaturalCommand(commandAnalysis, message, chat);
